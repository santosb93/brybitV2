...
const SignUp = () => {
  ...
  const signup = () => {
    ...
    const passwordConfirm = document.getElementById('Signup__confirm_password').value;
    ...
    */
    ...
    if (!username) {
      setUsernameMessage([<Message color = "red" message = 'Username field is required'/>]);
      return;
    }
    ...
    if (checkUsername()) {
      setUsernameMessage([<Message color = "red" message = 'Username already exists'/>]);
      return;
    }
    setUsernameMessage([]);
    /** 
    ...
    */
    if (!firstName || !lastName){
      setNameMessage([<Message color = "red" message = 'First and last Name field is required'/>]);
      return;
    }
      setNameMessage([]);
    ...
    */
    ...
    if (!password || !passwordConfirm){
      setPasswordMessage([<Message color = "red" message = 'Password field is required'/>]);
      return;
    }
    if (password.length < 6){
      setPasswordMessage([<Message color = "red" message = 'Passwords must be at least 6 characters long'/>]);
      return;
    }
    // confirm passwords are equal
    if (password !== passwordConfirm){
      setPasswordMessage([<Message color = "red" message = 'Passwords do not match'/>]);
      return;
    }
      setPasswordMessage([]);
    ...
    */
    if (!email || !emailConfirm){
      setEmailMessage([<Message color = "red" message = 'Emails field is required'/>])
      return;
    }
    if (email !== emailConfirm){
    ...
    <section id="signup_form">
      <form>
        ...
        <div className = "container">
          ...
          <input type = "text" id = "Signup__username" placeholder="Enter username"></input>
          {usernameMessage}
          {successMessage}
        </div>
        <div className = "container">
      ...
      <button id = 'signup_button' onClick ={signup}>Signup</button>
