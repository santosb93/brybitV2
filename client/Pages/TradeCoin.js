    ...
    if (orderMargin.value * orderValue.value > state.currentUser.brybits) {
      setErrorMessage([
        <Message
          ...
          key={1}
          color='red'
          message={`Not Enough Brybits. This order is equal to ${
            orderMargin.value * orderValue.value
          } brybits`}
        />,
      ]);
      return;
    ...
    if (state.activeTrade.length === 1) {
      setErrorMessage([
        <Message
          ...
          id='ErrorMessage'
          key={1}
          message='There is an active trade'
        />,
      ]);
      return;
    }
    ...
    <section className='TradeCoin'>
      <div id='TradeCoin_form'>
        ...
        <div className='container container--buttons'>
          <button id='short' onClick={trade}>
            Short
          </button>
          <button id='long' onClick={trade}>
            Long
          </button>
        </div>
        {errorMessage}
